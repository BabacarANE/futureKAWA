...
async def get_lots(token: str):
    return await fetch(f"{settings.api_colombie}/lots/", token)

async def get_measures(token: str, warehouse_id: int | None = None):
    url = f"{settings.api_colombie}/measures/"
    if warehouse_id:
        url += f"?warehouse_id={warehouse_id}"
    return await fetch(url, token)

async def get_alerts(token: str):
    return await fetch(f"{settings.api_colombie}/alerts/", token)

async def get_health(token: str):
    return await fetch(f"{settings.api_colombie}/health", token)
